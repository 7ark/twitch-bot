...
import {
    ...
    getRandomIntI,
    getRandomItem,
    GiveExp, HandleLoadingSession, IsAudioPaused,
    LoadDragonData, LoadPlayerSession, okayVoices, MuteAudio,
    playTextToSpeech, ResumeAudio, SavePlayerSession,
...
function onKeyReleased(key: KeyCode) {
    switch (key) {
        case KeyCode.F17:
            console.log("Toggling audio pause")
            if(IsAudioPaused()) {
                ResumeAudio();
            }
            else {
            ...
            break;
        case KeyCode.KEY_H:
            if(heldKeys.includes(KeyCode.LEFT_CTRL)) {
                console.log("yes")
            }
            break;
    }
...
let heldKeys: Array<KeyCode> = [];

const v = new GlobalKeyboardListener();
v.addListener(function (e, down) {
    if(e.state === "UP") {
        if(heldKeys.includes(e.scanCode)){
            heldKeys.splice(heldKeys.indexOf(e.scanCode), 1);
        }
        // console.log("Pressed " + e.scanCode);
        onKeyReleased(e.scanCode);
    }
    else {
        if(!heldKeys.includes(e.scanCode)){
            heldKeys.push(e.scanCode);
        }
    }
});
